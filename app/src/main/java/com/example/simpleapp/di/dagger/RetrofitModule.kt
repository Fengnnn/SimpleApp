...
class RetrofitModule {
    ...
    @Provides
    @Singleton
    fun provideMovieApiService(baseRetrofitBuilder: BaseRetrofitBuilder) =
        MovieRetrofitService(baseRetrofitBuilder).buildMovieService()

    @Provides
    @Singleton
    fun provideMovieObservable(movieApiService: IMovieApiService) =
        TMDbObservableBuilder(movieApiService).getMoviesObservable()
}
